...
public class MouseListener {
    ...
    public static void mousePositionCallback(long window, double XPos, double YPos) {
        ...
        // Get to know if mouse is being dragged, as one of the buttons would be pressed while dragging
    ...
    public static void mouseButtonCallback(long window, int button, int action, int mods) {
        if (action == GLFW_PRESS) {
            ...
            if (button < get().mouseButtonPressed.length) {
                ...
                get().isMouseDragging = false;
            }
        }
    }

    public static void mouseScrollCallback(long window, double xOffset, double yOffset) {
        // Set scrolling offsets to current values
        get().scrollX = xOffset;
        get().scrollY = yOffset;
    }
